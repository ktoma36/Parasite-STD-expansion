...
namespace rjwstd
{
	...
	public static class std_spreader
	{
			...
			foreach (std_def sd in std.all)
			{
				if (!catcher.health.hediffSet.HasHediff(sd.hediff_def))
				{
					if (catcher.health.immunity.GetImmunity(sd.hediff_def) <= 0.0f)
					{
						...
						if (STD_Settings.std_show_roll_to_catch)
						...
						if (catch_rv < catch_chance)
						{
							...
							{
								if (STD_Settings.std_sex) 
								{ 
									if (get_severity(pitcher, sd) >= STD_Settings.std_min_severity_to_pitch)
									{
										pitch_source = xxx.get_pawnname(pitcher);
										pitch_chance = 1.0f;
									}
								}
								if (STD_Settings.std_floor && pitch_chance != 1.0f)
								{
		...
		static float GetCleanlinessFactor(Pawn catcher)
		{
			...
			float cle = room?.GetStat(RoomStatDefOf.Cleanliness) ?? STD_Settings.std_outdoor_cleanliness;
			float exa = cle >= 0.0f ? STD_Settings.std_env_pitch_cleanliness_exaggeration : STD_Settings.std_env_pitch_dirtiness_exaggeration;
		...
		public static void generate_on(Pawn p)
		{
			...
			if (!xxx.is_human(p))
				return;
			float nymph_mul = !xxx.is_nympho(p) ? STD_Settings.pawn_spawn_with_std_mul : STD_Settings.nymph_spawn_with_std_mul;
			//Rand.PopState();
			//Rand.PushState(RJW_Multiplayer.PredictableSeed());
			foreach (std_def sd in std.all)
